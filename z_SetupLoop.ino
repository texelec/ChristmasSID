...
/////////////////////////////////////////////////////////////////////////////////////////////////
  noInterrupts();                                //Added by Kevin Williams for Christmas SID
  pinInit();                                     //Set LED output pins
  pinMode(AMP_MUTE,OUTPUT);                      //line for mute amp mute
  digitalWrite(AMP_MUTE,LOW);                    //LOW=Mute
  ITimer.attachInterruptInterval(1000, TimerHandler);   //Start Timer for LED Processing
  ISR_Timer.setInterval(4, Process_LEDs);        //update 250 times a second
  interrupts();                                  //end added code
/////////////////////////////////////////////////////////////////////////////////////////////////  
}
...
void loop() {
  ...
  if ( player == true ) { // go player
    ...
    if (instructions < uint32_t (500000)) { // detect sid init jam, or player's overload
      if (mode_play_ON) {
        digitalWrite(AMP_MUTE,HIGH);                  //Added by Kevin Williams for Christmas SID - Enable amp
        exec6502(); // execute 1 instruction
        //Emu_uS = Emu_uS + (ticktable[opcode]); // get emulated uS value
      }
      else {
        digitalWrite(AMP_MUTE,LOW);                  //Added by Kevin Williams for Christmas SID - disable amp
    ...
    if (JSR1003 == 1) { // JSR1003 check
      ...
      checkButton_PLAY();
      checkButton_NEXT();
      checkButton_LIGHT();  //added by Kevin Williams for Christmas SID
