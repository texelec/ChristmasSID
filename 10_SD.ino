...
//Added by Kevin Williams for Christmas SID
uint8_t  updatespeed=1;                         //default update flame @ 4ms - 0=disable
uint8_t  patternsel=1;                          //default=pattern 1, 0=off
...
bool ReadSettings () {
  ...
  if (sd.exists("STM32SID_Player_settings.bin")) {
    ...
    if (sidfile.open("STM32SID_Player_settings.bin", O_READ )) {
      ...
      patternsel  = uint8_t (sidfile.read());  //added by Kevin Williams for Christmas SID
      updatespeed = uint8_t (sidfile.read());  //added by Kevin Williams for Christmas SID
...
void WriteSettings (uint32_t p, uint32_t m , uint32_t fl, uint32_t fd, uint32_t ts) {
  ...
  if (sidfile.open("STM32SID_Player_settings.bin", O_RDWR)) { //  If O_CREAT and O_EXCL are set, open() shall fail if the file exists.
    ...
    sidfile.write(uint8_t(patternsel));  //added by Kevin Williams for Christmas SID
    sidfile.write(uint8_t(updatespeed)); //added by Kevin Williams for Christmas SID
